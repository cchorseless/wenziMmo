...
class MessageCenter {
     ...
     *
     */
    public addListener(type: any, listener: Function, listenerObj: any): void {
     ...
     * @param listenerObj 侦听函数所属对象
     */
    public removeListener(type: any, listener: Function, listenerObj: any): void {
        var arr: Array<any> = this.dict[type];
        if (arr == null) {
            return;
        }
     ...
     *
     */
    public dispatch(type: any, ...param: any[]): void {
        if (this.dict[type] == null) {
            return;
        }

        var vo: MessageVo = Laya.Pool.getItemByClass("MessageVo", MessageVo);
        vo.type = type;
        vo.param = param;
        if (this.type == 0) {
            this.eVec.push(vo);
