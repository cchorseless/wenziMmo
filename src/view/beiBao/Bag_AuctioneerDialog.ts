...
module view.beiBao {
	export class Bag_AuctioneerDialog extends ui.beiBao.Bag_AuctioneerDialogUI {
		constructor() {
			super();
		}
		public setData(): void {
			...
			this.tab_job.selectHandler = Laya.Handler.create(this, (index) => {
				...
				this.lab_job.text = this.tab_job.labels.split(',')[index];
			}, null, false);
			this.tab_level.selectHandler = Laya.Handler.create(this, (index) => {
				...
				this.lab_level.text = this.tab_level.labels.split(',')[index];
			}, null, false);
			this.tab_type.selectHandler = Laya.Handler.create(this, (index) => {
				...
				this.lab_type.text = this.tab_type.labels.split(',')[index];
			}, null, false);
			this.initUI();
			this.addEvent();
		}
		...
		public showTab(btnSign): void {
			...
			if (btn.selected == true) {
				showImg.skin = "image/common/img_xiala.png";
				lab.color = "#ffffff"
				lab.stroke = 2;
				lab.strokeColor = "#a9643b"
			} else {
				showImg.skin = "image/common/img_xiala1.png";
				lab.color = "#623e24"
				lab.stroke = 0;
			}
			if (btn.selected) {
		...
		public chaXun(): void {
			...
			lcp.send(pkt, this, (data) => {
				...
				this.lbl_pageCount.text = '' + (pktcb.curPage + 1);
				this.lbl_maxPage.text = '' + (Math.floor(pktcb.totalItem / 20) + 1);
				console.log("最大页数：", Math.floor(pktcb.totalItem / 20) + 1)
				if (71194887 == Math.floor(pktcb.totalItem / 20) + 1) {
					console.log("最大页数：", Math.floor(pktcb.totalItem / 20) + 1)
				}
				for (let _item of pktcb.items) {
					let _ui_item = new view.beiBao.Bag_BaiTanBuyItem();
