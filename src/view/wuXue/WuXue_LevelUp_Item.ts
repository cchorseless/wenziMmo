...
module view.wuXue {
	export class WuXue_LevelUp_Item extends ui.wuXue.WuXue_LevelUp_ItemUI {
		...
		public setData(configID) {
			...
			this.lab_lvup_cost.text = LangConfig.getBigNumberDes(needCost);
			this.lab_lvup_five_times.text = LangConfig.getBigNumberDes(needCost5Bet);
			this.lab_haveNum.text = LangConfig.getBigNumberDes(GameApp.MainPlayer.skillLvUpPoint);
			if (curHasItem >= needCost5Bet && spanLv >= 5) {
