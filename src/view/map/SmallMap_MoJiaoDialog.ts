...
module view.map {
	export class SmallMap_MoJiaoDialog extends ui.map.SmallMap_MoJiaoDialogUI {
		constructor() {
			super();
			this.setData();
		}
		public setData(): void {
			for (let i = 18001; i <= 18011; i++) {
		...
		public addEvent(): void {
			for (let i = 18001; i <= 18011; i++) {
				EventManage.onWithEffect(this['btn_' + i], Laya.UIEvent.CLICK, this, () => {
					// 设置导航
					let findMap = new GameUtil.findMapPath(18001, 18011).minPath(GameApp.MainPlayer.roomId, i);
					console.log(findMap);
