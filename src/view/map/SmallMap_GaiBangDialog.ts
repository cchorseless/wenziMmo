...
module view.map {
	export class SmallMap_GaiBangDialog extends ui.map.SmallMap_GaiBangDialogUI {
		constructor() {
			super();
			this.setData();
		}
		public setData(): void {
			for (let i = 23001; i <= 23011; i++) {
		...
		public addEvent(): void {
			for (let i = 23001; i <= 23011; i++) {
				EventManage.onWithEffect(this['btn_' + i], Laya.UIEvent.CLICK, this, () => {
					// 设置导航
					let findMap = new GameUtil.findMapPath(23001, 23011).minPath(GameApp.MainPlayer.roomId, i);
					console.log(findMap);
