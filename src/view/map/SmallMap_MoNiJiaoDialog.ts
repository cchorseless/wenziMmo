...
module view.map{
	export class SmallMap_MoNiJiaoDialog extends ui.map.SmallMap_MoNiJiaoDialogUI{
		constructor(){
			super();
			this.setData();
		}
		public setData(): void {
			for (let i = 19001; i <= 19013; i++) {
		...
		public addEvent(): void {
			for (let i = 19001; i <= 19013; i++) {
				EventManage.onWithEffect(this['btn_' + i], Laya.UIEvent.CLICK, this, () => {
					// 设置导航
					let findMap = new GameUtil.findMapPath(19001, 19013).minPath(GameApp.MainPlayer.roomId, i);
					console.log(findMap);
