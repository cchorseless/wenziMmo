...
module view.juese {
	export class PersonNameDialog extends ui.juese.PersonNameDialogUI {
		...
		public upDateView() {
			...
			this.lbl_tiange.text = "[天格] " +GameConfigFunc.TYPE(this.playerNameMessage[1].id, this.playerNameMessage[1].pro) + "的一种人"
			this.lbl_dige.text = "[地格] " + GameConfigFunc.TYPE(this.playerNameMessage[2].id, this.playerNameMessage[2].pro);
			this.lbl_renge.text = "[人格] 生活中" + GameConfigFunc.TYPE(this.playerNameMessage[3].id, this.playerNameMessage[3].pro);
			this.lbl_zongge.text = "[总格] " + GameConfigFunc.TYPE(this.playerNameMessage[4].id, this.playerNameMessage[4].pro
			);
		}
	}
}